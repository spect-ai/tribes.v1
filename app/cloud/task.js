...
Moralis.Cloud.define("updateTask", async (request) => {
  ...
  try {
    if (task.get("status") !== getStatusCode(request.params.task.status)) {
      task = await handleStatusChange(task, request.user.id, request.params.task.status, request.params.columnId);
...
async function handleStatusChange(task, userId, status, currentColumnId) {
  const logger = Moralis.Cloud.getLogger();
  try {
    const board = await getBoardByObjectId(task.get("boardId"));
    var columnsData = board.get("columns");
    const currentColumn = columnsData[currentColumnId];
    const index = currentColumn.taskIds.indexOf(task.taskId);
    logger.info(index);
    logger.info(JSON.stringify(currentColumn));
    const startTaskIds = Array.from(currentColumn.taskIds); // copy
    startTaskIds.splice(index, 1);
    logger.info(startTaskIds);
    const newStart = {
      ...currentColumn,
    ...
    logger.info(JSON.stringify(newColumn));
    const finishTaskIds = Array.from(newColumn.taskIds); // copy
    finishTaskIds.splice(newColumn.taskIds.length, 0, task.get("taskId"));
    const newFinish = {
      ...newColumn,
    ...
    logger.info(JSON.stringify(columnsData));
