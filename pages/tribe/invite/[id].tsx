...
const InviteModal = () => {
  ...
  const { isAuthenticated, Moralis, user, authenticate, isAuthenticating } =
    useMoralis();
  ...
  useEffect(() => {
    if (id) {
      decryptData(String(id));
      checkMemberInTeam(Moralis, teamId, String(user?.id))
        .then((res: any[]) => {
  ...
  const handleClickInvite = () => {
    addMemberToTribe(
      ...
      .then((res: any[]) => {
        ...
        if (
          String(res) == "member already exist" ||
          String(res) == "invite accepted"
        ) {
          router.replace(`/tribe/${teamId}`);
      ...
      <Fade in={showModal} timeout={500}>
        <Box sx={modalStyle}>
          <Wrapper>
            {isAuth ? (
              <Container>
                <Text>You Got Invited To Team {teamId}</Text>
                <PrimaryButton
                  variant="outlined"
                  fullWidth
